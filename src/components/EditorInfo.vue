<template>
  <div class="flex items-center">
    <div
      class="flex flex-col justify-center pr-4 mr-4 text-right border-r border-neutral-200 dark:border-neutral-800"
    >
    ...
    <div v-if="collabState === 'connected'" class="flex flex-row items-center">
      <div class="relative flex flex-row items-center ml-3">
        <div v-for="user in users.slice(0, 3)" :key="user.clientId" class="-ml-3">
        ...
        <div v-if="users.length > 3" class="-ml-3">
          <div
            class="flex items-center justify-center w-8 h-8 font-bold text-xs leading-none border border-white dark:border-black bg-[#FFA2A2] rounded-full"
          >
            +{{ users.length - 3 }}
          </div>
        </div>
